...
function resolveSheetPart(
    ...
    const ridMatch = workbookXml.match(
        new RegExp(`name="${sheetName}"[^>]*r:id="([^"]+)"`),
    );
    if (!ridMatch)
        throw new Error(`Sheet "${sheetName}" not found in workbook`);
    const rid = ridMatch[1];
    const targetMatch = relsXml.match(
        new RegExp(`Id="${rid}"[^>]*Target="worksheets/([^"]+)"`),
    );
...
function updateAppXml(appXml: string, newSheets: NewSheet[]): string {
    ...
    const entries = newSheets
        .map((s) => `<vt:lpstr>${escXml(s.name)}</vt:lpstr>`)
        .join("");
...
function patchCx(cxXml: string, round: Round): string {
    ...
    for (let i = 0; i < maxRows; i++) {
        ...
        if (cells)
            insertedRows += `<row r="${rowNum}" spans="1:8">${cells}</row>`;
    }
...
function rebuildWorkbookXml(
    ...
    const existingSheets =
        originalXml.match(/<sheets>([\s\S]*?)<\/sheets>/)?.[1] ?? "";
    const newSheetEntries = newSheets
        .map(
            (s) =>
                `<sheet name="${escXml(s.name)}" sheetId="${s.sheetId}" r:id="${s.rid}"/>`,
        )
        .join("");
...
export function buildXlsx(
    ...
    const exportSheets = buildExportSheets(round, opts).filter(
        (es) => es.sheet.kind !== "cx",
    );
    ...
    const usedNames = new Set(
        [...workbookXml.matchAll(/<sheet [^>]*name="([^"]*)"/g)].map((m) =>
            m[1].toLowerCase(),
        ),
    );
    ...
    files["xl/workbook.xml"] = strToU8(
        rebuildWorkbookXml(workbookXml, newSheets),
    );
