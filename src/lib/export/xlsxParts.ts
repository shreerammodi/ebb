 ...
 */
export function buildFlowSheetXml(
    templateXml: string,
    es: ExportSheet,
): string {
    ...
    const sheetData =
        templateXml.match(/<sheetData>[\s\S]*?<\/sheetData>/)?.[0] ??
        "<sheetData></sheetData>";
...
export function registerSheetsInWorkbook(
    workbookXml: string,
    sheets: NewSheet[],
): string {
...
export function registerSheetsInRels(
    relsXml: string,
    sheets: NewSheet[],
): string {
    const entries = sheets
...
export function registerSheetsInContentTypes(
    ctXml: string,
    sheets: NewSheet[],
): string {
    const entries = sheets
