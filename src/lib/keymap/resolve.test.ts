...
describe("resolveCommand (vim)", () => {
  ...
  it("i in normal mode → edit.enter", () => {
    expect(resolveCommand(VIM_KEYMAP, "normal", ev("i"))).toBe("edit.enter");
  });
  ...
  it("Ctrl+k in normal mode → sheet.quickSwitch", () => {
    expect(resolveCommand(VIM_KEYMAP, "normal", ev("k", { ctrlKey: true }))).toBe(
      "sheet.quickSwitch",
    );
  });

  it("O (shift+o) in normal mode → arg.newRoot (not Shift+O)", () => {
    expect(resolveCommand(VIM_KEYMAP, "normal", ev("O", { shiftKey: true }))).toBe("arg.newRoot");
  });
