...
describe("teamCode", () => {
    ...
    it("orders two debaters by last-name initial alphabetically", () => {
        ...
        expect(teamCode("Westwood", d("Al", "Smith"), d("Bo", "Jones"))).toBe(
            "Westwood JS",
        );
    });

    it("keeps order when already alphabetical", () => {
        expect(teamCode("Westwood", d("Al", "Adams"), d("Bo", "Baker"))).toBe(
            "Westwood AB",
        );
    });

    it("falls back to first+last initial of the single debater present", () => {
        expect(teamCode("Westwood", d("Carol", "Diaz"), d("", ""))).toBe(
            "Westwood CD",
        );
    });

    it("uses the second debater alone if first is empty", () => {
        expect(teamCode("Westwood", d("", ""), d("Carol", "Diaz"))).toBe(
            "Westwood CD",
        );
    });
