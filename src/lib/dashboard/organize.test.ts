...
describe("sortSummaries", () => {
    ...
    it("sorts by last edited desc", () => {
        expect(sortSummaries(list, "updated").map((x) => x.id)).toEqual([
            "b",
            "a",
        ]);
    });
    it("sorts by tournament asc", () => {
        expect(sortSummaries(list, "tournament").map((x) => x.id)).toEqual([
            "b",
            "a",
        ]);
    });
    it("sorts by date desc", () => {
        expect(sortSummaries(list, "date").map((x) => x.id)).toEqual([
            "b",
            "a",
        ]);
    });
