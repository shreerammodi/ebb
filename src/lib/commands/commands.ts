  ...
  switch (id) {
    ...
    case "format.toggleBold": {
      ...
      const node = round.nodes.find(
        (n) => n.sheetId === sel.sheetId && n.speechId === sel.speechId && n.row === sel.row,
      );
      if (!node) return;
      if (id === "format.toggleBold") {
        state.toggleNodeBold(node.id);
      } else {
        state.toggleNodeStatus(node.id, id === "status.toggleConceded" ? "conceded" : "extended");
      }
      return;
    }
    ...
    case "move.commit": {
      ...
      const dCol =
        columnsForSheet(round.format, round.sheets.find((s) => s.id === sel.sheetId)!).findIndex(
          (s) => s.id === sel.speechId,
        ) -
        columnsForSheet(
          round.format,
