 ...
 */
export function nextOpposingSpeech(
    format: Format,
    speechId: string,
): Speech | null {
...
export function adjacentInColumn(
    ...
    if (!cur) return null;
    const sameCol = placed.filter(
        (p) => p.col === cur.col && p.node.id !== nodeId,
    );
    if (dir === "up") {
        const above = sameCol.filter((p) => p.startRow < cur.startRow);
        if (above.length === 0) return null;
        return above.reduce((best, p) =>
            p.startRow > best.startRow ? p : best,
        ).node;
    }
    const below = sameCol.filter((p) => p.startRow > cur.startRow);
    if (below.length === 0) return null;
    return below.reduce((best, p) => (p.startRow < best.startRow ? p : best))
        .node;
}
