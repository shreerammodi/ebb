...
describe("GuideDialog", () => {
  ...
  it("opens to Welcome and switches sections", async () => {
    ...
    render(<GuideDialog />);
    expect(screen.getByTestId("guide-content")).toHaveTextContent(/Ebb is a keyboard-first tool/i);
  ...
  it("ArrowDown on section rail advances the active section", async () => {
    ...
    expect(screen.getByTestId("guide-content")).toHaveTextContent(/dashboard lists every flow/i);
  });
});
