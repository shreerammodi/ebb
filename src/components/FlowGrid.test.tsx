...
function setupScenario() {
    ...
    useRoundStore.getState()._commit(null, (rr) => ({
        ...rr,
        nodes: r.nodes.map((n) => {
            if (n.id === ac1Id || n.id === ac2Id || n.id === ac3Id)
                return { ...n, parentId: ncId };
            return n;
        }),
    }));
...
describe("FlowGrid — coordinate-based rendering", () => {
    ...
    it("highlights the selected cell with .cell-sel", () => {
        ...
        // Find ac2's row
        const ac2 = useRoundStore
            .getState()
            .round!.nodes.find((n) => n.id === ac2Id)!;
        useRoundStore
            .getState()
            .setSelection({ sheetId, speechId: s2AC, row: ac2.row });
...
describe("FlowGrid — CX sheet folding", () => {
    ...
    it("renders CX nodes with numbering (no special-case suppression)", () => {
        ...
        useRoundStore.getState()._commit(null, (rr) => ({
            ...rr,
            nodes: rr.nodes.map((n) =>
                n.id === rId ? { ...n, parentId: qId } : n,
            ),
        }));
