...
function setupTwoSheets() {
    ...
    const sheet1Id = useRoundStore
        .getState()
        .addSheet({ title: "Case", group: "aff" });
    const sheet2Id = useRoundStore
        .getState()
        .addSheet({ title: "Topicality", group: "neg" });
...
describe("PrintView", () => {
    ...
    it("renders sheets sorted by order", () => {
        ...
        expect(titles.indexOf("Case")).toBeLessThan(
            titles.indexOf("Topicality"),
        );
    });
});
