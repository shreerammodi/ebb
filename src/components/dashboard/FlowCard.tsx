    ...
    >
      ...
      <div className="flex items-center justify-between gap-2">
        <span className="pr-7 text-[15px] font-semibold tracking-tight text-pretty">
          <span className={cn(affBlank ? "text-muted-foreground italic" : "text-aff")}>{aff}</span>
          <span className="px-1.5 text-[13px] font-normal text-muted-foreground">vs</span>
          <span className={cn(negBlank ? "text-muted-foreground italic" : "text-neg")}>{neg}</span>
        </span>
        <span
          className={cn(
      ...
      <div className="grid grid-cols-[78px_1fr] gap-x-3 gap-y-1.5 text-[12.5px]">
        ...
        <span className={summary.round ? "" : "text-muted-foreground"}>{summary.round ?? "—"}</span>
        <span className="text-muted-foreground">Judge</span>
        <span className={summary.judge ? "" : "text-muted-foreground"}>{summary.judge ?? "—"}</span>
      ...
      {snippet && (
        <p className="mt-3 line-clamp-2 text-[12px] text-muted-foreground">
          {snippet.map((seg, i) => (
            <span key={i} className={seg.match ? "font-semibold text-foreground" : ""}>
              {seg.text}
            </span>
          ))}
        </p>
      ...
      <hr className="-mx-5 my-3 border-zinc-100" />
      <div className="text-[12px] text-muted-foreground">edited {edited}</div>
    </div>
  );
}
